...
import { BrowserSession, PageSnapshot } from "../browser/context.js";
import { classifyError, ErrorType, ClassifiedError } from "./detector.js";
import { diagnose, Diagnosis } from "./diagnoser.js";
 ...
 */
async function capturePageState(page: Page): Promise<{ url: string; contentHash: string }> {
  try {
 ...
 */
function shouldVerify(context?: { selector?: string; intent?: string; url?: string }): boolean {
...
let sharedHealer: SemanticHealer | null = null;

export function getSharedHealer(): SemanticHealer {
  if (!sharedHealer) {
    const stateDir = process.env.PHOENIX_STATE_DIR ?? path.join(process.cwd(), ".phoenix");
    fs.mkdirSync(stateDir, { recursive: true });
    sharedHealer = new SemanticHealer(
      new RepairCache({ persistPath: path.join(stateDir, "repair-cache.json") }),
      new WeightStore({ persistPath: path.join(stateDir, "weights.json") })
    );
  }
  return sharedHealer;
}
...
export async function executeWithHealing<T>(
  ...
  operation: (page: Page, effectiveSelector?: string) => Promise<T>,
  options: {
    maxRetries?: number;
    llm?: LLMProvider;
    context?: { selector?: string; url?: string; intent?: string };
    healer?: SemanticHealer;
    verify?: (page: Page, result: T, effectiveSelector?: string) => Promise<boolean | undefined>;
  } = {}
  ...
  let healMethod: HealingResult<T>["healMethod"];
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    ...
    try {
      ...
      if (currentSelector && options.context?.selector && currentSelector !== options.context.selector) {
        await healer.rememberSuccessfulRepair(
          session.getActivePage(),
          options.context.selector,
          currentSelector,
          options.context.intent ?? "the target element",
          healingConfidence
        );
        if (verifiedRepair) {
      ...
      try { snapshot = await session.getSnapshot(); } catch {}
      ...
      lastDiagnosis = await diagnose(
        classified,
        snapshot,
        classified.type === ErrorType.SELECTOR_NOT_FOUND ? undefined : options.llm
      );
