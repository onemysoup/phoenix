...
export interface RepairRule {
  ...
  visualFingerprint?: {
    ...
    parentRelX: number; parentRelY: number; isAboveFold: boolean;
    shape: "square" | "wide" | "tall" | "tiny"; bgColor: string; textColor: string; borderColor: string;
  };
...
export class RepairCache {
   ...
   */
  async validate(rule: RepairRule, page: { locator: (selector: string) => { count: () => Promise<number>; first: () => { isVisible: () => Promise<boolean> } } }): Promise<boolean> {
    try {
  ...
  import(json: string): void {
    try {
      ...
      for (const [key, rule] of entries.slice(-this.maxSize)) {
        if (!rule || typeof rule.expiresAt !== "number" || Date.now() > rule.expiresAt) continue;
        this.cache.set(key, rule);
      }
    } catch {}
  }
