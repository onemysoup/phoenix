...
export class SemanticHealer {
  ...
  private async multiDimensionalMatch(
    ...
    const elements = await page.evaluate(() => {
      ...
      const results: Array<{
        ...
        visual: {
          ...
          bgColor: string; textColor: string; shape: string;
        } | null;
    ...
    for (const el of elements) {
      ...
      if (query.expectedId && sem.id !== query.expectedId && !sem.id.includes(query.expectedId)) continue;
      ...
      if (el.visual) {
        const v = el.visual;
        if (query.expectClickable && (sem.isClickable || v.shape === "wide")) visScore += 0.5;
        if (query.expectEditable && sem.isEditable) visScore += 0.5;
        if (v.isAboveFold) visScore += 0.3;
...
Respond in JSON:
{
  ...
  private cacheRepair(domain: string, original: string, healed: string, intent: string, confidence: number): void {
