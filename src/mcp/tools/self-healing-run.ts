...
import { createLLMProvider, LLMProvider } from "../../healing/llm-provider.js";
...
function getLLM(): LLMProvider | null {
...
export async function handleSelfHealingRun(
  session: BrowserSession,
  params: z.infer<typeof selfHealingRunSchema>
) {
  const llm = params.useLLM ? (getLLM() ?? undefined) : undefined;

  const result = await executeWithHealing(
    session,
    async (page, effectiveSelector) => {
      ...
      switch (params.action) {
        ...
        case "click": {
          if (!sel) throw new Error("selector is required for click");
          await page.locator(sel).click({ timeout: 5000 });
          return { clicked: sel };
        }
        case "type": {
          if (!sel) throw new Error("selector is required for type");
        ...
        case "extract": {
          if (!sel) throw new Error("selector is required for extract");
        ...
        case "wait_for": {
          if (!sel) throw new Error("selector is required for wait_for");
    ...
    },
    {
      ...
      verify: async (page, _result, effectiveSelector) => {
        if (params.expectedUrl && page.url() !== assertNavigationAllowed(params.expectedUrl).toString()) return false;
        if (params.expectedSelector && !await page.locator(params.expectedSelector).first().isVisible().catch(() => false)) return false;
        if (params.expectedText && !await page.locator("body").innerText().then((text) => text.includes(params.expectedText!)).catch(() => false)) return false;
        if (params.expectedUrl || params.expectedSelector || params.expectedText) return true;
        switch (params.action) {
          case "navigate":
