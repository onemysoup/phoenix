...
export const spaMonitorSchema = z.object({
  action: z
    ...
    .describe(
      "'start' begins monitoring route changes, 'stop' stops monitoring, 'get_changes' returns collected changes"
    ),
});

export async function handleSpaMonitor(
  session: BrowserSession,
  params: z.infer<typeof spaMonitorSchema>
) {
  ...
  switch (params.action) {
    case "start": {
      ...
      await page.evaluate(() => {
        if ((window as any).__phoenix_route_monitor) return; // already running
        ...
        (window as any).__phoenix_route_monitor = {
          changes,
          getChanges: () => [...changes],
          clearChanges: () => {
            changes.length = 0;
    ...
    case "stop": {
      await page.evaluate(() => {
        const monitor = (window as any).__phoenix_route_monitor;
        monitor?.stop?.();
        delete (window as any).__phoenix_route_monitor;
      });
      return {
        content: [
          {
