...
export async function handleInfiniteScroll(
  ...
  params: z.infer<typeof infiniteScrollSchema>
) {
  const page = session.getActivePage();

  const getContentCount = async (): Promise<number> => {
  ...
  for (let i = 0; i < params.maxScrolls; i++) {
    ...
    if (newItems > 0) {
      ...
      stableRounds++;
      if (stableRounds >= params.stableRounds) break;
    }
  }
    ...
    content: [
      {
        ...
        text: JSON.stringify({
          ...
          stoppedBecause:
            stableRounds >= params.stableRounds ? "no_new_content" : "max_scrolls_reached",
        }),
      },
    ],
  };
