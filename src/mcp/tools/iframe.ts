...
export const switchFrameSchema = z.object({
  action: z
    .enum(["into", "back", "list"])
    .describe(
      "'into' switches into an iframe, 'back' returns to parent frame, 'list' lists all iframes"
    ),
  selector: z
    .string()
    .optional()
...
export async function handleSwitchFrame(
  ...
  params: z.infer<typeof switchFrameSchema>
) {
  ...
  switch (params.action) {
    ...
    case "into": {
      ...
      // We use a page-level data attribute to track current frame
      await page.evaluate(
        (sel) => {
          (window as any).__phoenix_current_frame = sel;
        },
        params.selector
      );
    ...
    case "back": {
      await page.evaluate(() => {
        delete (window as any).__phoenix_current_frame;
      });
      return {
        content: [
          {
