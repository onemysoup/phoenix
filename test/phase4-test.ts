...
async function main() {
  ...
  // Close one session first to make room, then create 6th
  await callTool(client, "session_close", { sessionId: sessionIds[0] });
  sessionIds.shift();

  // Now create a new session (should succeed immediately since we freed a slot)
  const s6 = await callTool(client, "session_create", { url: `http://localhost:${SERVER_PORT}/` });
  assert(!!s6.sessionId, `6th session created: ${s6.sessionId || s6.error}`);
