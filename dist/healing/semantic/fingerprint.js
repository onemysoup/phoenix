...
export function extractFingerprint(el) {
    ...
    if (id) {
        ...
        if (label)
            nearbyLabels.push(normalizeText(label.textContent ?? ""));
    }
...
function inferRole(tag, el) {
    if (tag === "button" || (tag === "input" && el.type === "button"))
        return "button";
...
function isVisuallyHidden(el) {
    ...
    if (clip && clip.startsWith("inset(")) {
        ...
        if (/^(100%|0px)\s+(100%|0px)\s+(100%|0px)\s+(100%|0px)/.test(inner))
            return true;
    }
    return false;
}
 ...
 */
export function extractAllFingerprints() {
    const all = document.querySelectorAll("*");
    const results = [];
    for (let i = 0; i < all.length; i++) {
        // Skip CSS-hidden elements (opacity:0, transform:scale(0), etc.)
        if (isVisuallyHidden(all[i]))
            continue;
        const fp = extractFingerprint(all[i]);
        // Skip empty/invisible elements
        if (!fp.text && !fp.id && !fp.name && !fp.ariaLabel && !fp.placeholder && fp.classes.length === 0)
            continue;
