...
import { visualSimilarity } from "./visual-anchor.js";
...
export class SemanticHealer {
     ...
     */
    async multiDimensionalMatch(page, brokenSelector, intent, referenceVisual) {
        const domain = new URL(page.url()).hostname;
        ...
        for (const el of elements) {
            ...
            const visScore = el.visual && referenceVisual
                ? visualSimilarity(referenceVisual, el.visual)
                : 0;
...
Respond in JSON:
{
  ...
  "reasoning": "how you identified the element"
    ...
    async cacheRepair(page, domain, original, healed, intent, confidence) {
        const visualFingerprint = await page.locator(healed).first().evaluate((el) => {
            ...
            const shape = rect.width < 10 && rect.height < 10 ? "tiny" : ratio > 3 ? "wide" : ratio < 0.33 ? "tall" : "square";
            return {
    ...
    async rememberSuccessfulRepair(page, original, healed, intent, confidence) {
        const domain = new URL(page.url()).hostname;
    ...
    fingerprintHash(selector) {
        ...
        for (let i = 0; i < selector.length; i++)
            hash = Math.imul(hash ^ selector.charCodeAt(i), 16777619);
