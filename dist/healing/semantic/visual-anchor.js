 ...
 */
export function extractVisualFingerprint(el) {
    const htmlEl = el;
    ...
    if (tf && tf !== "none") {
        ...
        if (m) {
            ...
            if (vals.length >= 4 && (Math.abs(vals[0]) < 0.001 || Math.abs(vals[3]) < 0.001))
                return null;
        }
    }
...
export function visualSimilarity(a, b) {
    ...
    const posDist = Math.sqrt(Math.pow(a.relX - b.relX, 2) + Math.pow(a.relY - b.relY, 2));
    const posSim = Math.max(0, 1 - posDist * 5); // 0.2 distance = 0 score
    score += posSim * 3.0;
    weight += 3.0;
    // 2. Size similarity
    const sizeDist = Math.sqrt(Math.pow(a.relWidth - b.relWidth, 2) + Math.pow(a.relHeight - b.relHeight, 2));
    ...
    const layoutDist = Math.sqrt(Math.pow(a.parentRelX - b.parentRelX, 2) + Math.pow(a.parentRelY - b.parentRelY, 2));
 ...
 */
export function extractAllVisualFingerprints() {
    const all = document.querySelectorAll("*");
    const results = [];
    for (let i = 0; i < all.length; i++) {
        const vf = extractVisualFingerprint(all[i]);
        if (vf && vf.isVisible)
            results.push({ index: i, visual: vf });
    }
    return results;
}
//# sourceMappingURL=visual-anchor.js.map