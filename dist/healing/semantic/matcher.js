 ...
 */
export function buildQueryFromSelector(selector, intent) {
    const query = {};
    // Parse ID selector: #my-id
    const idMatch = selector.match(/^#([\w-]+)$/);
    if (idMatch)
        query.expectedId = idMatch[1];
    // Parse class selector: .my-class
    const classMatch = selector.match(/^\.([\w-]+)$/);
    if (classMatch)
        query.expectedClasses = [classMatch[1]];
    // Parse tag selector: button, input, a
    const tagMatch = selector.match(/^(button|input|a|select|textarea|div|span|form)$/i);
    if (tagMatch)
        query.expectedTag = tagMatch[1].toLowerCase();
    // Parse compound: tag.class, tag#id, tag[attr=val]
    const compoundMatch = selector.match(/^(\w+)(?:\.([\w-]+))?(?:#([\w-]+))?/);
    if (compoundMatch) {
        if (compoundMatch[1] && !query.expectedTag)
            query.expectedTag = compoundMatch[1].toLowerCase();
        if (compoundMatch[2] && !query.expectedClasses)
            query.expectedClasses = [compoundMatch[2]];
        if (compoundMatch[3] && !query.expectedId)
            query.expectedId = compoundMatch[3];
    }
    // Infer behavior from selector patterns
    if (/button|btn|submit|click/i.test(selector))
        query.expectClickable = true;
    if (/input|text|search|email|password/i.test(selector))
        query.expectEditable = true;
    if (/submit/i.test(selector))
        query.expectSubmit = true;
    // Use intent text
    if (intent)
        query.intentText = intent;
    return query;
}
/**
 ...
 */
export function scoreFingerprint(fp, query, weights) {
    ...
    if (query.originalFingerprint) {
        ...
        let attrCount = 0;
        if (orig.name) {
            attrScore += fp.name === orig.name ? 1 : 0;
            attrCount++;
        }
        if (orig.dataTestId) {
            attrScore += fp.dataTestId === orig.dataTestId ? 1 : 0;
            attrCount++;
        }
        if (orig.ariaLabel) {
            attrScore += fp.ariaLabel === orig.ariaLabel ? 1 : 0;
            attrCount++;
        }
        if (orig.placeholder) {
            attrScore += fp.placeholder === orig.placeholder ? 1 : 0;
            attrCount++;
        }
        if (attrCount > 0) {
 ...
 */
function getDynamicWeights(fp, query) {
