 ...
 */
export var AntiBotType;
(function (AntiBotType) {
    AntiBotType["CAPTCHA"] = "captcha";
    AntiBotType["IP_BLOCKED"] = "ip_blocked";
    AntiBotType["RATE_LIMITED"] = "rate_limited";
    AntiBotType["CLOUDFLARE_CHALLENGE"] = "cloudflare_challenge";
    AntiBotType["FINGERPRINT_DETECTED"] = "fingerprint_detected";
    AntiBotType["UNKNOWN"] = "unknown";
})(AntiBotType || (AntiBotType = {}));
// ========== Detection ==========
export async function detectAntiBot(page, snapshot) {
...
export function getAntiBotStrategy(detection) {
...
export async function applyAntiBotStrategy(page, strategy, context) {
    if (strategy.waitMs) {
...
export async function analyzeAntiBotWithLLM(detection, snapshot, llm) {
    const prompt = ANTIBOT_PROMPT
        .replace("{url}", snapshot.url)
        .replace("{pageHtml}", snapshot.html.slice(0, 4000))
