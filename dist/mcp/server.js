...
export function createMcpServer() {
    ...
    const terminatedSessionOrder = [];
    const maxTerminatedSessions = 10_000;
    const mutex = new SessionMutex();
    const auditLog = new AuditLog();
    const markSessionTerminated = (sessionId) => {
        ...
        if (terminatedSessionOrder.length > maxTerminatedSessions) {
            ...
            if (oldest)
                terminatedSessionIds.delete(oldest);
        }
    };
     ...
     */
    function withConcurrency(sessionId, operationName, fn) {
    ...
    // === Session management ===
    server.tool("session_create", "Create a new browser session. Returns a session ID.", { url: z.string().url().optional().describe("Initial URL to navigate to") }, async ({ url }) => {
    ...
    server.tool("session_close", "Close a browser session and release resources.", { sessionId: sessionIdParam }, async ({ sessionId }) => {
        return mutex.run(sessionId, async () => {
            ...
            return {
                content: [{ type: "text", text: JSON.stringify({ closed: sessionId }) }],
            };
        });
    });
    ...
    server.tool("get_page_info", "Get current page URL, title, and optionally full HTML.", { sessionId: sessionIdParam, includeHtml: z.boolean().default(false) }, async ({ sessionId, includeHtml }) => {
        return withConcurrency(sessionId, "get_page_info", async (session) => {
            ...
            const info = { url: snapshot.url, title: snapshot.title };
            if (includeHtml)
                info.html = snapshot.html;
    ...
    // === Self-healing tool (Phase 3) ===
    server.tool("self_healing_run", "Execute a browser action with automatic self-healing. When the action fails, the engine diagnoses the cause (using LLM if available) and retries with fixes. Supports: navigate, click, type, extract, wait_for.", { sessionId: sessionIdParam, ...selfHealingRunSchema.shape }, async ({ sessionId, ...params }) => {
        return withConcurrency(sessionId, "self_healing_run", (session) => handleSelfHealingRun(session, params));
    });
    // === Phase 4: Pool stats ===
    server.tool("pool_stats", "Get pool and concurrency statistics: active sessions, queue depth, utilization, error rate.", {}, async () => {
        const stats = pool.getMetrics();
        return {
            content: [{ type: "text", text: JSON.stringify(stats) }],
        };
    });
    server.tool("health", "Return MCP service and browser-pool readiness. The server is ready before a browser is lazily launched.", {}, async () => ({ content: [{ type: "text", text: JSON.stringify(pool.health()) }] }));
    server.tool("audit_events", "Return recent bounded audit events. Error text is included; do not expose this tool to untrusted tenants.", { limit: z.number().int().min(1).max(500).default(100), sessionId: z.string().optional() }, async ({ limit, sessionId }) => ({ content: [{ type: "text", text: JSON.stringify({ events: auditLog.recent(limit, sessionId) }) }] }));
    // Cleanup
    const cleanup = async () => {
        await pool.shutdown();
        process.exit(0);
